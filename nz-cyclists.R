...
nz.cyclists <- rbind(nz.cyclists, data.frame(ratio=n90(fatality.rate.per.cyclist),year=year,measure="Fatality rate per cyclist"))
...
nz.cyclists <- rbind(nz.cyclists, data.frame(ratio=n90(police.reported.injuries.rate.per.cyclist),year=year,measure="Fatality rate per cyclist"))
nz.cyclists <- rbind(nz.cyclists, data.frame(ratio=n90(police.reported.injuries.rate.per.min),year=year,measure="Fatality rate per minute cycling"))
nz.cyclists <- rbind(nz.cyclists, data.frame(ratio=n90(police.reported.injuries.rate.per.km),year=year,measure="Fatality rate per km cycled"))
...
nz.cyclists <- rbind(nz.cyclists, data.frame(ratio=n90(non.motor.vehicle.hosp.rate.per.cyclist),year=year,measure="Non-motor vehicle accident cyclist hospitalisation rate per cyclist"))
...
nz.cyclists <- rbind(nz.cyclists, data.frame(ratio=n90(motor.vehicle.hosp.rate.per.cyclist),year=year,measure="Motor vehicle accident cyclist hospitalisation rate per cyclist"))
...
nz.cyclists <- rbind(nz.cyclists, data.frame(ratio=n90(all.hosp.rate.per.cyclist),year=year,measure="All cyclist hospitalisation rate per cyclist"))
...
p <- ggplot(nz.cyclists) + geom_line(aes(x=year,y=ratio,colour=measure)) + geom_vline(xintercept=1994,color="red")
p
